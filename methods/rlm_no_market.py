...
class RLMNoMarketForecaster(ForecastMethod):
    ...
    def _fallback_uniform(self, example: Example) -> List[float]:
        """
        Fallback to uniform distribution when prediction extraction fails.

        NOTE: In ablation mode (no-market), we intentionally stripped belief[] arrays
        from the current market, so we CANNOT use last-price fallback. Instead, we
        return uniform distribution and log a warning.

        This makes fallback failures clearly visible in metrics (uniform = no info).
        """
        n = len(example.options)
        self._diagnostics.log(
            f"FALLBACK: Extraction failed for {example.event_id}, returning uniform 1/{n}. "
            "This indicates the model didn't produce a valid prediction.",
            level="WARNING"
        )
        return [1.0 / n] * n
...
Do this NOW. Output ONLY the code block above with your actual probability values."""
        ...
        return self._fallback_uniform(example), stats
    ...
    def predict(self, batch: Batch, spec: Dict[str, Any]) -> List[List[float]]:
        ...
        for i, example in enumerate(batch.examples):
            ...
            try:
                ...
                if self.verbose:
                    print(f"[RLM] Error: {e}")
                pred = self._fallback_uniform(example)
...
# =============================================================================
# NOTES (rlm_no_market.py)
# =============================================================================
#
# PURPOSE: Ablation - test RLM WITHOUT current market prices (crowd wisdom)
#
# KEY DIFFERENCES FROM rlm_forecaster.py:
# - build_setup_code() strips belief[] arrays from current market
# - _fallback_uniform() returns [1/n]*n (not last price)
# - System prompt warns "NO CURRENT MARKET PRICES"
#
# DOS:
# - Use robust extraction (_extract_prediction_robust) - 4 strategies
# - Return uniform on fallback (makes failures visible in Brier)
# - Log WARNING when fallback used
#
# DONTS:
# - Don't use _fallback_last_price() - belief arrays are stripped
# - Don't rely solely on FINAL_VAR - model often fails to call it right
#
# EXPERIMENTS + RESULTS:
# - 2026-01-22: Fallback rate 57% -> 5% after robust extraction fix
# - 2026-01-22: Brier 0.645 on test (model struggles without prices - expected)
# - Prompt-only fixes got ~50% success; code-level fix got ~95%
#
# THINGS THAT DIDNT WORK:
# - Prompt changes alone (model inconsistent at following FINAL_VAR pattern)
# - Relying on FINAL_VAR output only (misses predictions in response text)
#
# IMPLEMENTATION NUANCES:
# - Retry logic uses new RLM instance with max_iterations=3
# - Extraction tries: var assignment -> keywords -> standalone arrays -> percentages
# - Absolute paths required for parquet (REPL runs in temp dir)
#
